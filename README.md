...
- Gives each production PostgreSQL idempotency claim a database-timed 300-second lease. An expired `processing` row is reclaimed only by an atomic compare-and-set over its status, opaque ownership token, and stored acquisition timestamp; reclaim issues a fresh token and cannot downgrade completed or concurrently replaced work.
