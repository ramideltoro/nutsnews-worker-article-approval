    ...
    createPrometheusRuntimeTelemetrySink: () => ({
      ...
      collect(): string {
        const lines = [
          ...
          'nutsnews_worker_inflight{environment="production",service="approval",queue="nutsnews.worker.approval.v1",outcome="in_flight"} 0'
        ];
        ...
        if (runtime1Delegate.health.size > 0) {
          ...
          for (const [probe, current] of runtime1Delegate.health) {
            for (const outcome of [
              ...
              lines.push(`nutsnews_worker_health_probe{environment="production",service="approval",probe="${probe}",outcome="${outcome}"} ${outcome === current ? "1" : "0"}`);
            }
          }
        }
      ...
      setInFlight(): void {
        // Runtime1-like delegate method required by the pinned runtime interface.
      },
...
describe("approval Runtime1 health metric compatibility", () => {
  ...
  it("keeps one custom health-probe family when the delegate supports the Runtime1 family", async () => {
    ...
    expect(lines).toContain('nutsnews_worker_health_probe{environment="production",service="approval",probe="readiness",outcome="ok"} 1');
    expect(lines).toContain('nutsnews_worker_inflight{environment="production",service="approval",queue="nutsnews.worker.approval.v1",outcome="in_flight"} 0');
  });
});
