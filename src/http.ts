...
export interface ApprovalHttpServer {
...
export function createApprovalHttpServer(options: ApprovalHttpServerOptions): ApprovalHttpServer {
  ...
  return {
    ...
    url: (path = "/") => {
      ...
      if (!isAddressInfo(address)) {
        throw new Error("Approval HTTP server is not listening on a TCP address.");
      }
...
async function routeRequest(
  options: ApprovalHttpServerOptions,
  request: http.IncomingMessage,
  response: http.ServerResponse
): Promise<void> {
  const url = new URL(request.url ?? "/", "http://localhost");

  if (request.method === "POST" && url.pathname === APPROVAL_RECONCILIATION_PATH) {
  ...
  switch (url.pathname) {
    ...
    case "/config-schema":
      writeJson(response, 200, {
        ...
        variables: APPROVAL_CONFIG_SCHEMA
      });
      return;
    default:
...
async function handleReconciliationRequest(
  ...
  if (options.reconciler === undefined || options.reconciliationToken === undefined) {
    writeJson(response, 503, {
      ...
      errors: [
        "approval reconciliation endpoint is not configured"
      ]
    });
    return;
  }
  ...
  let body: ApprovalReconciliationRequest;
...
function readJsonBody(request: http.IncomingMessage): Promise<ApprovalReconciliationRequest> {
  const chunks: Buffer[] = [];
  let totalBytes = 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("end", () => {
      try {
        ...
        resolve(parsed as unknown as ApprovalReconciliationRequest);
      } catch {
...
function writeHealth(
  response: http.ServerResponse,
  report: Awaited<ReturnType<ApprovalService["health"]["liveness"]>>
): void {
